...
| v2.5 startup preflight supervisor ownership | Implemented. `/api/system/preflight` now starts with a supervisor ownership check, and the dashboard power-on animation shows the same check before administrator login. |
...
| v2.5 planning/dispatch shadow persistence | Implemented. Production plans, dispatch tasks, and upper-level allocation orders now have PostgreSQL/SQLite shadow tables, startup recovery into `MemoryStore`, and replay-readiness/persistence status coverage. |
| v2.5 alert/AI/audit shadow recovery | Implemented. Alerts, AI diagnoses, and manual audit logs are restored into `MemoryStore` on startup, alert state transitions are written back to persistence, and replay-readiness now checks alert/AI/audit shadow coverage. |
...
- startup preflight: `/api/system/preflight` now reports supervisor ownership as the first check; live verification showed 8/8 supervised processes healthy before login
...
- planning/dispatch recovery: central-api tests verify production plans, dispatch tasks, and allocation orders survive a new store instance through shadow tables
- alert/AI/audit recovery: central-api tests verify closed alerts, AI raw responses, and operator audit logs survive a new store instance through shadow tables
