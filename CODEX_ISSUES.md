...
| 14d | Run-scoped operational fact persistence | Resolved for v2.5. Alerts, AI diagnoses, commands, audit events, command shadow rows, and part queue shadow rows now persist `run_id`; startup migrates existing SQLite/PostgreSQL schemas, and replay prefers exact `run_id` matches with timestamp fallback for legacy rows. |
| 14e | Planning and dispatch shadow persistence | Resolved for v2.5. Production plans, dispatch tasks, and upper-level allocation orders now write to shadow tables, are restored on startup, and are included in persistence/replay-readiness checks. |
| 14f | Alert, AI, and audit runtime recovery | Resolved for v2.5. Alerts, AI diagnoses, and manual audit logs now restore into `MemoryStore` on startup; alert confirmations, diagnoses, closures, decisions, and escalation approvals write state back to persistence; tests verify recovery after a new store instance. |
